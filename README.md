...
Анекдоты записать в data.json (UTF-8 без BOM). При помощи функции recursive_split мы разбиваем тексты на примерно одинаковые кластера. Функция delete_dubles делает очистку от совсем похожих анекдотов, но функция будет пропускать одинаковые анекдоты оформленные по разному. 
